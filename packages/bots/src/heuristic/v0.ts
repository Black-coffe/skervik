// S2.4.1 — the canonical v0 bot brain (moved, behavior-preserving, from the
// S1.7.2 E2E scripted TEST driver at `packages/server/src/e2e/scriptedDriver.ts`).
// Given an authoritative `GameState` and a seat id, `decideAction` returns the
// single LEGAL intent that seat should send next, or `null` when it is not
// that seat's move. It is a pure function of state: fixed move priority,
// deterministic tie-breaks (topology / resource-name order), NO `Math.random`
// — determinism is what makes a bot-driven match reproducible/replayable. The
// same function drives both the in-process harness (`../harness.ts`) and, via
// the `Bot` seam (`../bot.ts`), any future server-side bot seat.
//
// Termination is the caller's contract (a hard step/turn cap): a greedy legal
// player that also bank-trades surplus into what it needs makes monotonic VP
// progress (resources only accumulate across turns; a turn's own actions
// strictly spend), so SOME seat reaches the Classic 10-VP threshold. The
