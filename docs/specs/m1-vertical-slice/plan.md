...
| S1.7.2 CI E2E full 3–4-client scripted match | ✅ **DONE — merged to main `4679323` (2026-07-07; T3 Opus×2 + lead-review verdict MERGE).** Phase A = match-start auto-fire on seats-full through the authoritative `#queue`/persist/broadcast pipeline (`dd2db41`). Phase B = cross-process socket E2E: 3 real `@colyseus/sdk@0.17` clients play a full Classic match to `game.ended` (winner 10 VP, 278 steps) via a deterministic expansion-first TEST driver, asserting **(1)** victory (`computeVictoryPoints`≥10), **(2)** run-twice determinism (canonicalized + boot-free raw-deep-equal backstop), **(3)** replay-equality (persisted `events.ndjson`→`parseGameEventLog`→`replay` deep-equal to authoritative `gameState`), **(4)** seed-secrecy end-to-end (driver `2b57ee8`, harness `77311be`). ≥1 bank trade exercised; hard step/time caps fail loudly. Named CI step "E2E full-match gate" (A2: one-event log flip fails exactly it). Core byte-unchanged. **Finding→memory:** greedy city-first driver deadlocks at 3 VP — the core economy is finite+deflationary (bank never refills), so expansion-first + purposeful roads are mandatory to reach 10 VP. 3 non-blocking review nits noted in the story (exact-string seed-scan, naive `canonicalize` relabel, `boot.ts` seed-seam not in Files) — carry into S1.7.3 if convenient. |
| S1.7.3 seed reveal + `GET /matches/{id}/verify` | ✅ **DONE — merged to main `f4ef796` (2026-07-07; T3 Opus, TWO lead-review rounds). CLOSES THE M1 GATE.** The **Verify** half of commit-reveal («Честный жребий»). **Core:** pure `verifyMatchRandomness(seed, events)` recomputes EVERY M1 seed-derived draw (dice `DICE_A/B`, robber-steal `STEAL`, dev-card `shuffledDevDeck`, board `generateBoard`) via the real primitives vs logged facts, in the deterministic core (no server-side duplication → no stream-index drift); `reduce`/`validate` byte-unchanged, zero-dep. **Server:** `FsMatchMetadataStore` writes `{matchesDir}/{matchId}/seed-reveal.json` (sidecar, OUT of the ndjson — Fork 3); `GET /matches/:id/verify` = thin wrapper: `sha256Hex(seed)===match.started.seedHash` (server crypto) AND core verify → JSON verdict; requires `game.ended` before serving the seed (never leaks mid-match → 409/404). Named CI "Fairness verify gate". **CRITICAL FINDING→memory ([[commit-reveal-verify-positional-binding]]):** the first verifier keyed draws off log-author-controlled fields (`event.index`/`deckRemaining`) → a dishonest server could grind them to forge favorable dice/steals/`victoryPoint` dev-cards and still get `ok:true` (lead-review EMPIRICALLY proved it against the real build). Fix (`6fbdf88`) anchors every draw to the verifier's OWN contiguity+deck counters (true folded position), NOT self-reported fields; re-review confirmed all 5 forgeries now rejected + honest match passes. Residual (documented): seed+log alone can't detect a seamless re-indexed OMISSION/insertion — needs a signed/hash-chained log (post-M1). DEFER: client verify UI (S3.3.3/M3), analytics (S3.4.1), Postgres, endpoint auth/rate-limit. **Non-blocking nits carried:** no test drives a REAL played-out match log through `verifyMatchRandomness` (cheap regression guard vs future index/deck-model drift); `verifyMatchRandomness` folds untrusted log w/o try-guard (pathological log → route 500 not clean `ok:false`). |
| **🎯 M1 VERTICAL SLICE COMPLETE (2026-07-07)** | E1.1–E1.7 all merged. 3–4 people play a fair, deterministic, seed-verifiable Classic match online (real WS boot, guest auth, authoritative validate→reduce→broadcast pipeline, event-sourced log, trade UI, commit-reveal RNG + public `/verify`). **NEXT = M2** (rule profiles + adaptive duration + catch-up + reconnect/grace/bot-fill + matchmaking/lobby/Redis presence + heuristic bots). Carry into M2: the 3 S1.7.3 nits above; the S1.7.2 review nits (exact-string seed-scan, naive `canonicalize`); a signed/hash-chained log for full omission-proof verification. |
