...
| S1.5.1 protocol zod schemas + server inbound validation | ⏳ dispatched (T2, isolated worktree) — zod schemas for envelope `{v,type,payload}` + `PlayerIntent`, **pinned to core types** (drift guard, EventLogLine lesson); server swaps ad-hoc guard → `safeParse`; zod becomes protocol's first runtime dep; core stays zero-dep. Client wiring = E1.6 |
| S1.5.2 handshake/versioning | ⏸ pending (T1) — `auth.connect` / `room.state`, protocol version negotiation; after S1.5.1 |
