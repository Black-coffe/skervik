...
| S1.7.3 seed reveal + `GET /matches/{id}/verify` | ✅ **materialized — DISPATCH-READY (S1.7.2 merged `4679323`, dep cleared)** `S1.7.3-seed-reveal-verify.md` (T3 Opus + lead-review). The **Verify** half of commit-reveal + M1-gate closer («Честный жребий»). **Core:** additive pure `verifyMatchRandomness(seed, events)` recomputes EVERY M1 seed-derived draw (dice `DICE_A/B`, robber-steal `STEAL`, dev-card `shuffledDevDeck`, board `generateBoard`) via the real primitives vs the logged facts — verification math in the deterministic core (NOT duplicated server-side → the #1 stream-index-drift risk); `reduce`/`validate` byte-unchanged, zero-dep. **Server:** `FsMatchMetadataStore` durably writes `{matchesDir}/{matchId}/seed-reveal.json` (sidecar, OUT of the ndjson — Fork 3); `GET /matches/:id/verify` = thin wrapper: `sha256Hex(seed)===match.started.seedHash` (server crypto) AND core verify → JSON verdict. Reveal ONLY on `game.ended` (S1.4.3 untouched); NEVER leaks seed mid-match (404/409). Named CI "Fairness verify gate" (A2: tamper fails exactly it). DEFER: client verify UI (S3.3.3/M3), analytics (S3.4.1), Postgres, endpoint auth/rate-limit. **Closes the M1 GATE.** |
