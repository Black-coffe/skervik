...
| S1.6.3 HUD (Instruments layer) | 🔨 materialized (T3), **dispatch after S1.6.6a merge (now done)**. Layout §6 + `zustand` store + §11 primitives (Button/Pill/PlayerCard/Panel/PhaseIndicator/TimerDisplay/SeedChip/EmptyState) + top bar (SeedChip/phase/timer) + players rail + bottom deck (my hand) + log placeholder. **CRUX from core scout:** `computeVictoryPoints` NOT exported + includes hidden VP; `player.victoryPoints` also hidden-inclusive → **derive PUBLIC Renown in-client** (settlements+cities×2+awards from exported `CLASSIC_VICTORY_PROFILE`, NO opponent hidden VP), pure+tested; opponents show hand SIZE only, ventures via exported `publicDevCardCount`. Fixes S1.6.2 nit-2 (fixture negatives) + folds 3 S1.6.6a nits. Defers: log content, §8 dashboard/histogram, Trade UI (S1.6.4), interactivity/WS/live-timers (S1.6.5), mobile. **Follow-up flagged:** consider exporting `publicVictoryPoints` from core (rules-in-core hygiene) to retire the client renown-mirror. |
| S1.6.4–S1.6.6 | not materialized. S1.6.4 Trade UI = product heart (T3, Opus + lead-review). S1.6.5 WS client (sets `myPlayerId`, live timers/connection, intent loop). S1.6.6 = residual locale content/QA (framework already shipped as S1.6.6a). |
