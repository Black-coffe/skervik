...
| S1.6.5 WS client (live wire) | ✅ **materialized — READY TO DISPATCH** (S1.6.4 merged `d99c1af`, dep cleared) `S1.6.5-ws-client.md`. Fills the `dispatchIntent` stub + builds on S1.6.4's store; must NOT alter Trade-UI components. Carries the 3 S1.6.4 review nits (esp. #1: real pending/echo reconciliation makes the dead incoming-`pending` prop live). T3 Opus + lead-review. Contract pinned to server/protocol: join with `{protocolVersion}` → `myPlayerId=room.sessionId` → `state.snapshot` seeds store → fold `event.batch` via core `reduce` (Fork 1, ordered by `eventIndex`) → `room.send('intent',envelope)` → reconcile `pendingSeal` on echo/reject (§7.6 server-truth). Adds `colyseus.js`; hoists `GAME_ROOM_NAME` to protocol. **Verified vs a MOCK transport** — runnable server boot (`listen()`), match-start orchestration, real cross-process E2E, reconnect grace, server timers, guest auth all **deferred to E1.7/M2** (noted in story). |
| S1.6.6 residual locale content/QA | not materialized (framework already shipped as S1.6.6a; residual = locale-string QA pass across all E1.6 surfaces). |
