...
| S1.6.3–S1.6.6 | not materialized. **S1.6.3 HUD = FIRST i18n consumer** → sequence S1.6.6 framework portion before, or materialize the `t()` contract at, S1.6.3 (ADR-0010 custom typed layer + `Intl`; missing-key = build fail). Display VP via `computeVictoryPoints` not `player.victoryPoints` (S1.3.4 nit). Heed S1.6.2 nit-2 (fixture negatives). S1.6.4 Trade UI = product heart (T3, Opus + lead-review). |
