...
| S1.4.3 commit-reveal (reveal + `sha256Hex`) | ⏳ dispatched (T3 Opus, isolated worktree) — seed reveal to match-metadata seam on `game.ended`, leak checklist, extract+unit-test `sha256Hex` (S1.4.1 nit) |
| S1.4.4a core log-format (bare-GameEvent + re-golden) | ⏳ dispatched (Opus, isolated worktree) — retire `EventLogLine`/`toGameEvent`, add `parseGameEventLog`, re-golden determinism fixture; auto-closes the S1.2.1 `RESOURCES_PRODUCED` nit |
| S1.4.4b server FsEventSink (durable log write) | ⏸ pending — after S1.4.2+S1.4.3+S1.4.4a; implements `GameEventSink` FS writer to `matches/{id}/events.ndjson` + **fixes S1.4.2 NIT-1 (persist-before-commit ordering) as a hard precondition** + folds NIT-2/3 |
