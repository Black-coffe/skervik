...
| S1.7.1 guest auth + server boot (real `listen()`) | ✅ **materialized — DISPATCH-READY** `S1.7.1-guest-auth-rooms.md` (T3 Opus + lead-review). **Right-sized:** boot + guest-auth + quick-match ONLY; **room-by-code split to S1.7.1b** (UX, not gate-critical — `joinOrCreate` already seats 4). Brings Fastify (ADR-0009 deferral resolved) + Colyseus WS transport on one port + `GET /health` + `POST /auth/guest` (anonymous, in-memory, NOT a security boundary in M1) + `startServer()`. **Guest identity = display-only; authoritative `PlayerId` stays `sessionId`** (GameRoom logic byte-untouched — scout's hard constraint). **🔴 STEP 0 STOP-GATE:** first task is a colyseus **version-compat spike** (0.16 client ↔ 0.17 server schema-sync — the S1.6.5 flag); if schema state-sync breaks across the version gap → worker STOPS, Queen brings `lead-architect` + ADR (version alignment), no hack. Real-socket smoke test = first cross-process proof (S1.7.2 scales to full match). |
...
| S1.7.2 CI E2E full 3–4-client scripted match | not materialized (needs S1.7.1 real boot; scripted plain-WS clients → victory, asserts server determinism + event-log replay-equality — the isomorphic Fork-1 fold's real cross-process proof). |
