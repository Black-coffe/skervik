...
| S1.4.4b server FsEventSink (durable log write) | ⏳ dispatched (Opus, isolated worktree) — `FsEventSink`→`matches/{id}/events.ndjson`, round-trip via `parseGameEventLog`+`replay`, **fixes S1.4.2 NIT-1** (persist-before-commit + crash-safe handler) + NIT-2/3. CLOSES E1.4 |
