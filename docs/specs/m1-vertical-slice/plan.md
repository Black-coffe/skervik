...
| S1.5.2 handshake/versioning | ⏳ dispatched (T1, isolated worktree) — client presents `protocolVersion` on connect; server `onAuth` rejects incompatible/malformed via Colyseus `ServerError` carrying typed `error.version` (`PROTOCOL_VERSION_MISMATCH`) BEFORE seating; compat rule = exact-match `PROTOCOL_VERSION` in one helper `isCompatibleProtocolVersion` (M2 loosens); `error.version` added to `ServerMessageSchema` (S1.5.1 drift-pins intact); existing `state.snapshot` serves ROADMAP's "room.state" (no dup); no client wiring (E1.6), no auth/accounts (S1.7.1), core untouched |
