...
| S2.6.2 guest/OAuth identity → `users` (+ JWT) | T3 | ▶ **NEXT** — first external-auth surface (`/security-review` before merge); persist guest into `users` (replace `InMemoryGuestStore`), gated on `DATABASE_URL` | — |
| E2.6 persistence & accounts (×5; S2.6.5 non-gating) | T2–T3 | ▶ **IN PROGRESS** — governed by **ADR-0012** (Drizzle+PGlite, accepted 2026-07-13). S2.6.1 ✅. Then S2.6.2 guest/OAuth→`users` · S2.6.3 Pg `MatchMetadataStore` rewire (folds S2.6.1 nits #1 `update` empty-patch guard + #2 pool.end) · S2.6.4 GDPR · S2.6.5 solo save/resume (non-gating) | — |
