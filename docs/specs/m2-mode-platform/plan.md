...
| 3 | **E2.3 Resilience: reconnect & bot-fill** | S2.3.1 grace ✅ · S2.3.2 in-tab resync ✅ · S2.3.2a reload-resume ✅ · **S2.3.3 bot-fill + safe leave/rejoin [T3]** | **3/4** — human-reconnect COMPLETE (`e8d3172`+`a9d42fd`+`e1c8d73`); S2.3.3 = last (bot-fill on grace expiry + deliberate safe-leave) | S2.3.3 consumes the S2.4.3 `#driveBotTurn` seam; verify server game-end emits a consented close (S2.3.2a nit #3) |
...
| **game-end consented-close** (S2.3.2a lead-review nit #3): the client clears its `current-room` reconnect pointer only on a CONSENTED (code 4000) leave, so "clear on clean game-end" relies on the SERVER closing a finished room with a consented close code. If game-end closes non-consented, a reload after the match wastes one resume attempt (server rejects → fallback join — harmless). | **S2.3.3** (verify/ensure server game-end / room disposal emits a consented close; client-side already correct) | S2.3.2a lead-review nit #3 |
...
| E2.3 reconnect & bot-fill (×4 incl. S2.3.2a) | T2–T3 | ⏳ **3/4** — S2.3.1 grace + S2.3.2 in-tab resync + S2.3.2a reload-resume done (human-reconnect COMPLETE); **S2.3.3 bot-fill on expiry + safe-leave [T3]** = LAST (consumes the S2.4.3 `#driveBotTurn` seam) | — |
