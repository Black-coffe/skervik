...
| S2.6.3 Pg-backed match metadata + `match_players` | T2 | ▶ **NEXT** — `PgMatchMetadataStore` writing `matches`/`match_players` at match-start + `game.ended`; extends `MatchMetadataStore`; `event_log_uri` → ndjson; folds S2.6.1 nits (#1 `update` empty-patch guard, #2 `pool.end` in migrate CLI) | — |
| E2.6 persistence & accounts | T2–T3 | ▶ **IN PROGRESS** — **ADR-0012** (Drizzle+PGlite). S2.6.1 ✅ · S2.6.2a ✅ · S2.6.2b deferred (owner OAuth creds) · **S2.6.3 next** · S2.6.4 GDPR · S2.6.5 solo save/resume (non-gating). Carry-forward nits: S2.6.2a review nit #2 (client retry discards token on transient outage → post-M2) + #3 (cold-load omits displayName echo → S1.7.1b) | — |
