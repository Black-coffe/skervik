...
| S2.1.7a 5–6 player expanded board — CORE | T3 | 🟡 **WIP on branch `feat/s2.1.7a-expanded-board-core`** (6 commits `1c63c96`→`c8cb7bd`; worker hit session limit ~90% done). DONE+committed: buildTopology(radius,portSlotCount) param + topologyForRadius memo · BoardProfile.radius + EXPANDED_BOARD/PROFILE (37/36/11) · G4 guard · validate per-match topology · **verify board-leg discharge** · block-F tests (board/boardgen/verify: 350 core green). **2 items REMAIN before merge:** (1) `pnpm -r typecheck` FAILS — registering EXPANDED_PROFILE widened `RuleProfileId` → client `LobbyScreen.tsx:29` exhaustive `Record<RuleProfileId>` needs an `expanded` entry + i18n keys `lobby.preset.expanded.{name,description}` ×ru/uk/en (⇒ registering a profile is NOT core-only; also protocol `ShippingProfileIdSchema` for lobby-select); (2) expanded-profile/G4 test in `ruleProfile.test.ts` (AC2) — worker left `EXPANDED_BOARD`/`EXPANDED_PROFILE` imported-but-unused, reverted to HEAD by Queen. **Resume:** finish those 2 → full verify → lead-review → merge. Core byte-frozen + core-deps-guard confirmed green | branch |
