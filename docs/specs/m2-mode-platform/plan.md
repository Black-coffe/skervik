...
| 3 | **E2.3 Resilience: reconnect & bot-fill** | S2.3.1 grace ✅ · S2.3.2 in-tab resync ✅ · **S2.3.2a resume-on-load [T2]** · **S2.3.3 bot-fill + safe leave/rejoin [T3]** | **2/4** — S2.3.1 (`e8d3172`) seat-hold + S2.3.2 (`a9d42fd`) client reconnect+resync (in-tab); S2.3.2a = page-reload resume | S2.3.2a makes the persisted token functional; S2.3.3 consumes the S2.4.3 `#driveBotTurn` seam |
...
| **S2.3.2a resume-on-load + retry budget [T2, follow-up]** | T2 | ▫ **NEXT** — owner-chosen follow-up (2026-07-08): wire `connect()`→read persisted (roomId,token)→`client.reconnect` BEFORE `joinOrCreate` so a PAGE RELOAD resumes the held seat (makes S2.3.2's sessionStorage layer functional, delivers the S2.3.2 Goal); widen the bounded retry to span more of the 120s grace. Client-only (server reclaim+resync already done). | — |
| E2.3 reconnect & bot-fill (×4 incl. S2.3.2a) | T2–T3 | ⏳ **2/4** — S2.3.1 grace + S2.3.2 in-tab resync done; **S2.3.2a resume-on-load [T2]** = next, then **S2.3.3 bot-fill on expiry + safe-leave [T3]** (consumes the S2.4.3 `#driveBotTurn` seam) | — |
