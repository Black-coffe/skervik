...
| **the genesis override rule lives in TWO places** (`m2-gate-08`): `GameRoom.#adaptiveVpToWinOverride` is `#private`, so `expandedMatch.e2e.test.ts:88` mirrors it in a local `adaptiveOverrideFor(profileId, seatCount)` helper. The two agree today, but the mirror does NOT reproduce the room's `ADAPTIVE_MIN/MAX_PLAYERS` seat-range guard, so a rule change (a second knob, a different absent-key condition, a seat-range change) must be edited in both or the e2e silently builds a genesis production no longer produces — the exact defect review M4 raised. Held together only by `GameRoom.test.ts`'s genesis legs pinning the room's own output. | fold into whatever story next changes the override rule — or export a shared pure helper from the room module and delete the mirror | `m2-gate-08` impl. note |
| **workspace packages resolve through `dist`, so a stale build masks core/protocol fixes** — observed independently by three workers this round: `m2-gate-02` (server reads core via `dist`), `m2-gate-06` (floor 5→8 invisible to server/client suites until `pnpm -r build`), `m2-gate-07` (the new wsClient test failed with `profileId: undefined` until `pnpm --filter @skervik/protocol build` — the C3 regression reproduced by a stale dist). Same class as the §0.5 rebuild rule, now proven for `@skervik/protocol` and for the CLIENT too: `packages/client/vitest.config.ts` takes precedence over `vite.config.ts`, and only the latter aliases `@skervik/*` to `src`, so the client's dev/build read source while its TESTS read `dist`. **Unresolved and worth an owner/CI look:** `dist/` is gitignored and `.github/workflows/ci.yml` runs `Build` as its LAST step — after the determinism gate, the E2E full-match gate, the fairness gate and `Test` — so on a clean CI checkout those steps run with no `dist` present at all. | confirm the CI step order (a `pnpm -r build` before the test steps, or a source alias in the package vitest configs); until then, rebuild before debugging any cross-package seam | §0.5 · `m2-gate-02`/`06`/`07` impl. notes |
...
| S2.1.7a 5–6 player expanded board — CORE | T3 | ✅ done — buildTopology(radius,portSlotCount) param + topologyForRadius memo · BoardProfile.radius + EXPANDED_BOARD/PROFILE (37/36/11) · G4 guard · validate per-match topology · **verify board-leg discharge** · block-F tests (board/boardgen/verify: 350 core green). History (was 🟡 WIP on `feat/s2.1.7a-expanded-board-core`, 6 commits `1c63c96`→`c8cb7bd`, worker hit session limit ~90% done): the 2 items that had remained were closed before merge — (1) `pnpm -r typecheck` FAILED because registering EXPANDED_PROFILE widened `RuleProfileId` → client `LobbyScreen.tsx:29` exhaustive `Record<RuleProfileId>` needed an `expanded` entry + i18n keys `lobby.preset.expanded.{name,description}` ×ru/uk/en (⇒ **registering a profile is NOT core-only**; also protocol `ShippingProfileIdSchema` for lobby-select — the drift that `m2-gate-03` later pinned by test); (2) expanded-profile/G4 test in `ruleProfile.test.ts` (AC2). Core byte-frozen + core-deps-guard green | `cac69af` |
| S2.1.7b 5–6 player — CONSUMERS | T3 | ✅ done — routed server(`GameRoom`)/client(`GameTable`)/bots through `topologyForRadius`; `maxSeats`→6 + lobby routes ≥5 to `expanded`; bots play the big board (owner-approved); 5–6p cross-process e2e (single + multi + verify: `expandedMatch.e2e.test.ts` + `expandedMultiClient.e2e.test.ts`). Last feature pack before the M2-gate audit. **`3035ba5` is the pack's LAST COMMIT, not a merge commit** (single parent `21add10`, a plan-delta/scope-stats docs commit) — cite it as the pack's tip; the genuine merge commit in this area is S2.1.7a's `cac69af` | `3035ba5` |
...
> **Amended 2026-08-17** (owner, `m2-gate/brief.md` `## Answers` — verbatim answers to Q1 and
> Q2 of the M2-gate audit briefing). Two clauses only: **presence** — «Амендить гейт —
...
| heuristic bots ×3 | `packages/bots/src/v1.test.ts` (difficulty reproducibility/determinism + hard ≥ easy trend), `bot.test.ts`, `eval.test.ts`, `harness.test.ts` — S2.4.1 `57efc8c` / S2.4.2 `62400ca` (+ core fix `8129962`) |
| accounts (guest JWT) + GDPR | Guest identity + JWT: `packages/server/src/auth/dbGuestStore.test.ts`, `guestStore.test.ts`, `sessionToken.test.ts` (S2.6.2a `8aa1fd6`, /security-review PASS). GDPR export + erasure: `packages/server/src/routes/account.test.ts` (S2.6.4 `7542700`, /security-review PASS). OAuth (S2.6.2b) is owner-blocked, per the amendment above |
