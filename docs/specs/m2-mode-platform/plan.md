...
| 3 | **E2.3 Resilience: reconnect & bot-fill** | S2.3.1 reconnect tokens + grace ≥120s ✅ · **S2.3.2 full state resync [T2]** · **S2.3.3 bot-fill + safe leave/rejoin [T3]** | **1/3** — S2.3.1 (`e8d3172`): native `allowReconnection` seat-hold, timer NOT paused, transport-only, no forfeit | consumes S2.1.4 anti-AFK flags; S2.3.3 consumes the S2.4.3 `#driveBotTurn` seam |
...
| E2.3 reconnect & bot-fill (×3) | T2–T3 | ⏳ **1/3** — S2.3.1 grace done; **S2.3.2 full state resync [T2]** = next (client re-attach + missed-state push), then **S2.3.3 bot-fill on expiry + safe-leave [T3]** (consumes the S2.4.3 `#driveBotTurn` seam) | — |
