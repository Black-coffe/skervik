...
| **robin-hood discount unreachable by the v1 bot** (S2.2.5 Diagnostic A): `bankTradeToward` always proposes `count: bestBankRate(...)`, the same fn `validate.ts:1984` checks against, so `intent.count !== rate` is never true. 0 discount trades in 3000 matches → `robinHood`/`eventTiles` are **not measured**, not "no effect". | **S2.4.4** (teach the bot to spend a token), then re-measure | [[bot-sim-blind-spots]] |
...
| S2.2.6 guards + honest preset docs | T2 | ▫ next — three asserts at registry init (`eventTilesInterval >= 1`; `eventTiles ⇒ robinHood`; `1 <= robinHoodExchangeRate < bankTrade.baseRate`), each with a failing-input test; remove the `__vp9_test__`/`__balanced_vp9_test__` scaffolding; honest per-preset claims. **No preset literal changes** — nothing earned one | — |
