...
| S2.6.4 GDPR export + erasure | T2 | ▶ **NEXT** — auth-gated (S2.6.2a token) self-service export + soft-delete/anonymize (retain `match_players`, tombstone user); works for guests; `/security-review` (PII surface). Story on main, dispatch-ready | — |
