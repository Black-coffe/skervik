...
| S2.6.1 Postgres schema + Drizzle migrations + repos | T2 | 🔨 **dispatched** — first DB surface; `users`/`matches`/`match_players` only (ADR-0012); schema+generated migrations+`runMigrations`+`migrate` script+3 typed repos; PGlite-hermetic tests; NO room rewire, `boot.ts` untouched, DB optional (no-regression); deps in `packages/server` only, core stays zero-dep | — |
| E2.6 persistence & accounts (×5; S2.6.5 non-gating) | T2–T3 | ▶ **IN PROGRESS** — governed by **ADR-0012** (Drizzle+PGlite, accepted 2026-07-13). S2.6.1 dispatched. Then S2.6.2 guest/OAuth→`users` · S2.6.3 Pg `MatchMetadataStore` rewire · S2.6.4 GDPR · S2.6.5 solo save/resume (non-gating) | — |
