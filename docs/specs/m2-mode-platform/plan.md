...
| S2.4.2 heuristic v1 ×3 difficulty | T3 | 🅿️ **PARKED then RESUMING** — eval module + v1 brain + BotRng + difficulty DONE at `5282e93` (branch `feat/s2.4.2-heuristic-v1`, v0 frozen, 8 tests `.skip`-blocked on the bank bug). Resume: merge main (bank fix) → unskip → seed-sweep should pass → lead-review | — |
| E2.3 reconnect & bot-fill (×3) | T2–T3 | ▫ not started | — |
| E2.4 bots (×3) | T2–T3 | ⏳ S2.4.1 done + S2.4.2a core-fix done; S2.4.2 (v1 ×3) parked-resuming · S2.4.3 single-player + bot-fill [T2] not started | — |
