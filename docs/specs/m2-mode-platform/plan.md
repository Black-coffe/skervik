...
| 2–6 players | `twoPlayerMatch.e2e.test.ts` (2p phantom), classic/balanced/blitz e2e at 4 seats, `expandedMatch.e2e.test.ts` + `expandedMultiClient.e2e.test.ts` (5–6p board) — S2.1.6 `f0d69b7`, S2.1.7a `cac69af`, S2.1.7b `3035ba5` |
...
| adaptive duration ≤60 min | LIVE at match genesis: `packages/server/src/room/GameRoom.test.ts` (genesis-override leg — expanded 5p → 8 VP, 6p → 6 VP; Classic 2–4p and twoPlayer emit nothing, 4p Classic = 58 min). Seam + honoring reads: `packages/core/src/reduce.test.ts` (`match.started` fold survives replay), `packages/core/src/ruleProfile.test.ts` (`vpToWinOverride` replaces the profile threshold), `packages/protocol/src/messages.test.ts` (wire schema), `packages/core/src/adaptiveDuration.test.ts` (S2.1.3 `f0738b6`) — `m2-gate-05` + `m2-gate-02` |
